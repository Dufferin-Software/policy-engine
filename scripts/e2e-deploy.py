...
Run with --help for all options. For nodes alone — an in-place package
upgrade, or adding one node to an existing fleet — use node-deploy.py instead;
the two share their node-side logic via pe_deploy.py.
"""
...
def main(argv: list[str]) -> int:
    ...
    node_debs = resolve_node_debs(deb_dir, args.feature, list(NODE_PKGS))
    ...
    try:
        ...
        token = mint_token(host, prefix="e2e")
        add_operator(args, host)
        create_bundle(
            host,
            args.controller_api,
            token,
            f"https://{bundle_host(args)}:7777",
            bundle_path,
            ttl=args.ttl,
        ...
        # A fleet redeploy always re-enrols: the controller DB was just wiped,
        # so every node needs a fresh identity against the new CA.
        plan = NodePlan(
            debs=node_debs,
            ...
            wipe_identity=True,
            hosts_pin=(
                None if args.no_hosts else (args.controller_host, args.controller_name)
            ),
            what=f"feature: {args.feature}",
        )
