...
// The "Traffic Rule" tab of Fleet Config: creates the same rule across every
// picked node/interface/direction target by fanning out the per-node createRule
// mutation once per target.
export default function FleetRuleTab({ nodes, onClose, onApplied }: Props) {
  ...
  async function doCreate() {
    ...
    try {
      ...
      onApplied(`Fleet rule created on ${targets.length} target${targets.length !== 1 ? 's' : ''}.`)
      onClose()
    } catch (e) {
  ...
  return (
    <>
      ...
      <form onSubmit={handleSubmit} className="flex-1 min-h-0 overflow-y-auto p-5 space-y-4">
        ...
        {/* ── Step 1: add targets ──────────────────────────────────────── */}
        <FleetTargetPicker nodes={nodes} mode="rule" targets={targets} onChange={setTargets} />
