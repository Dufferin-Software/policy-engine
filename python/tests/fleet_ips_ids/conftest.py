...
Topology: 1 controller VM + node1/node2 (suricata-capable) + plain (non-
suricata engine), all on a shared management network.
...
# node1/node2 are suricata-capable (policy-engine-ips); "plain" is a
# non-suricata engine used for capability-gating tests.
_MANAGED_NODES = ["node1", "node2", "plain"]
...
def controller_api_token(nodes, controller_service) -> str:
    ...
    token = mint_api_token(nodes["controller"], f"netsim-fleet-ips-{int(time.time())}")
