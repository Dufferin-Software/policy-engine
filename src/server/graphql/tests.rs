...
mod tests {
    ...
    async fn test_attach_tc_field_registered() {
        let body = get_mutation_fields().await;
        assert!(
            body.contains("attachEgress"),
            "Expected mutation 'attachEgress' in schema: {}",
            body
        );
    }
    ...
    async fn test_detach_tc_field_registered() {
        let body = get_mutation_fields().await;
        assert!(
            body.contains("detachEgress"),
            "Expected mutation 'detachEgress' in schema: {}",
            body
        );
    }
    ...
    async fn test_gql_direction_enum_in_schema() {
        ...
        let res = schema
            .execute(
                r#"{ __type(name: "GqlDirection") { kind enumValues { name } } }"#,
            )
            .await;
        assert!(res.errors.is_empty(), "Errors: {:?}", res.errors);

        let body = format!("{}", res.data);
        assert!(body.contains("ENUM"), "GqlDirection should be an ENUM type: {}", body);
        assert!(body.contains("INGRESS"), "GqlDirection should have INGRESS: {}", body);
        assert!(body.contains("EGRESS"), "GqlDirection should have EGRESS: {}", body);
    }
    ...
    async fn test_gql_policy_action_enum_in_schema() {
        ...
        let res = schema
            .execute(
                r#"{ __type(name: "GqlPolicyAction") { kind enumValues { name } } }"#,
            )
            .await;
        assert!(res.errors.is_empty(), "Errors: {:?}", res.errors);
        ...
        assert!(body.contains("TAIL_CALL"), "Should have TAIL_CALL: {}", body);
    }
    ...
    async fn test_delete_rule_input_has_direction_field() {
        ...
        let res = schema
            .execute(
                r#"{ __type(name: "DeleteRuleInput") { inputFields { name } } }"#,
            )
            .await;
        assert!(res.errors.is_empty(), "Errors: {:?}", res.errors);
    ...
    async fn test_default_action_input_has_direction_field() {
        ...
        let res = schema
            .execute(
                r#"{ __type(name: "DefaultActionInput") { inputFields { name } } }"#,
            )
            .await;
        assert!(res.errors.is_empty(), "Errors: {:?}", res.errors);
    ...
    async fn test_tail_call_input_has_direction_field() {
        ...
        let res = schema
            .execute(
                r#"{ __type(name: "TailCallInput") { inputFields { name } } }"#,
            )
            .await;
        assert!(res.errors.is_empty(), "Errors: {:?}", res.errors);
    ...
    async fn test_attach_tc_input_type_exists() {
        ...
        let res = schema
            .execute(
                r#"{ __type(name: "AttachTcInput") { inputFields { name } } }"#,
            )
            .await;
        assert!(res.errors.is_empty(), "Errors: {:?}", res.errors);
    ...
    async fn test_detach_tc_input_type_exists() {
        ...
        let res = schema
            .execute(
                r#"{ __type(name: "DetachTcInput") { inputFields { name } } }"#,
            )
            .await;
        assert!(res.errors.is_empty(), "Errors: {:?}", res.errors);
